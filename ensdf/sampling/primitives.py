...
def sample_along_spline(spline, points_on_curve, surface_normals, radius, num_offsets=5):
    device = surface_normals.device
    t_vals = torch.linspace(0, 1, points_on_curve.shape[0])  
    tangents = torch.tensor(spline.derivative(1)(t_vals), dtype=torch.float32, device=device)
    tangents[0] += 1e-4
    tangents = F.normalize(tangents, dim=1)
    tangents[0] = 0.0
    normals = torch.cross(tangents, surface_normals)
    radii_offsets = radius * (2*torch.rand((points_on_curve.shape[0], num_offsets - 1), device=device) - 1.0)  
    radii_offsets = torch.cat((radii_offsets, torch.zeros(points_on_curve.shape[0], 1, device=device)), dim=1)
    sampled_points = radii_offsets.unsqueeze(-1) * normals.unsqueeze(1)
    return sampled_points, radii_offsets