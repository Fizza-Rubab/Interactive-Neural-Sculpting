...
#     intersections = []
#     for normal, center in zip(plane_normal, plane_center):
#         # Calculate intersections for each plane normal and center
#         denom = (normal * directions).sum(-1, keepdim=True)
#         t = ((center - points) * normal).sum(-1, keepdim=True) / denom
#         intersections.append(points + t * directions)

#     # Stack the results or apply further processing if needed
...
def wave_fall(x):
    return torch.sin(2*torch.pi*x) + 1.0
...
def ellipse(x):
    a = 1.5 
    b = 0.8 
    z = torch.exp(-((x[:, 0]-0.4)/a)**2 - ((x[:, 1]+0.23)/b)**2) 
    return z
