...
class SDFEditingDataset(DatasetBase):
    ...
    def sample(self, num_samples=None):
        ...
        if self.num_model_samples != 0: 
            with record_function("Sample Surface"):
                self.model_samples = self.next_model_samples
                self.next_model_samples = next(self.sdf_sampler)
            # Model samples
            with record_function("Discard Stroke Samples"):
            ...
            with record_function("Sample Stroke Region"):
                inter_points, inter_sdf, inter_normals = (
                    self.brush.sample_interaction(self.model, num_interaction_samples)
                )
                
                sample_points  = torch.cat((filtered_model_points, inter_points.reshape(-1, 3)))
                sample_normals = torch.cat((filtered_model_normals, inter_normals.reshape(-1, 3)))
                sample_sdf     = torch.cat((filtered_model_sdf, inter_sdf.reshape(-1, 1)))
